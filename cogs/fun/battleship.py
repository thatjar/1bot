    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        ...
        if enemy.is_dead():
            ...
            if enemy_cell.button and enemy_cell.button.view:
                ...
                await view.message.edit(content="You lose :(", view=None)
...
Disabled red button → You hit the enemy ship successfully.
...
class ReadyButton(discord.ui.Button["Prompt"]):
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        ...
        if interaction.user.id != self.player.member.id:
            await interaction.response.send_message(
                "❌ This ready button is not for you.", ephemeral=True
            )
            return
